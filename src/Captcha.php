...
class Captcha
{
    ...
    public function __set(string $name, $value)
    {
        ...
        throw new InvalidArgumentException('Config attribute not found');
    }
     ...
     * @return mixed
     */
    public function create(string $app, string $id = '', $output = true)
    {
        // 图片宽(px)
        ...
        $this->_img = imagecreate($this->imageW, $this->imageH);
        // 设置背景
        ...
        // 设置字体
        if (empty($this->font)) {
            $this->font = ($this->type == 'zh' || $this->type == 'num') ? __DIR__ . '/ttf/zh.ttf' : __DIR__ . '/ttf/en.ttf';
        }
        // 绘制杂点
        if ($this->useNoise) {
            $this->writeNoise();
        ...
        switch ($this->type) {
            case 'zh':
                ...
                for ($i = 0; $i < $this->length; $i++) {
                    ...
                    imagettftext($this->_img, $this->fontSize, mt_rand(-40, 40), $this->fontSize * ($i + 1) * 1.5, $this->fontSize + mt_rand(10, 20), $this->_color, $this->font, $code[$i]);
                }
                break;
            case 'en':
                // 英文验证码
     ...
     * @return boolean
     */
    public function check(string $code, string $app, string $id = ''): bool
    {
        // 获取验证码
        $secode = $this->getCode($app, $id);
        if (empty($secode) || empty($code)) {
            $this->error = '验证码错误';
            return false;
        }
        // 验证有效期
        ...
        // 清除已使用验证码
        $this->reset && $this->deleteCode($app, $id);
        return true;
    }
