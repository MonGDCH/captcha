...
class Image implements CaptchaDrive
{
    ...
    public function create(array $typeArr = ['scalar'], int $length = 4): CaptchaInfo
    {
        ...
        switch ($type) {
            case 'zh':
                // 中文验证码
                for ($i = 0; $i < $length; $i++) {
                    $code[$i] = iconv_substr($this->zhSet, mt_rand(0, mb_strlen($this->zhSet, 'utf-8') - 1), 1, 'utf-8');
                    imagettftext($this->_img, $this->fontSize, mt_rand(-40, 40), intval($this->fontSize * ($i + 1) * 1.5), $this->fontSize + mt_rand(10, 20), $this->_color, $this->font, $code[$i]);
                }
                break;
            case 'en':
                // 英文验证码
     ...
     * @return string
     */
    protected function getFont(string $type): string
    {
        $dir = dirname(__DIR__) . '/ttf/';
        $lang = ($type == 'zh' || $type == 'num') ? 'zh.ttf' : 'en.ttf';
        return $dir . $lang;
    }
